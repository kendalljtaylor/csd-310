...
DROP USER IF EXISTS 'ktaylor'@'localhost';


-- create movies_user and grant them all privileges to the movies database 
CREATE USER 'ktaylor'@'localhost' IDENTIFIED WITH mysql_native_password BY 'kendall';

-- grant all privileges to the movies database to user movies_user on localhost 
GRANT ALL PRIVILEGES ON movies.* TO 'ktaylor'@'localhost';
